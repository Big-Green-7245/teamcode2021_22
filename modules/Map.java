...
public class Map
{
    ...
    public void GroupShapes(double buffer)
    {
        ...
        while(mapObjects.hasNext() || mapObjects.hasPrevious())
        {
            MapObject object = new MapObject();
            if (mapObjects.hasNext()) {
                object = mapObjects.next();
            }else if (mapObjects.hasPrevious()){
                object = mapObjects.previous();
            }
    ...
    public List<double[]> ClearPath(double[] startPoint, double[] endPoint, double buffer)
    {
        ...
        // Validate path for each shape
        for (int i = 0; i < MapObjects.size(); i++)
        {
            path = ClearShapePath(i, path, buffer, false);
        }
    ...
    public class Circle extends MapObject
    {
        ...
        {
            // Weird 0.0000001 thing because function was messing up with rounding errors
            return ((position[0]- super.XPos)*(position[0]-super.XPos) + (position[1]-super.YPos)*(position[1]-super.YPos) < Radius*Radius-0.0000001);
        }
